...
struct MemoryEditor
{
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr)
    {
        ...
        s.PosHexStart = (s.AddrDigitsCount + 5) * s.GlyphWidth;
...
#define _PRISizeT   "zX"
    ...
    void DrawContents(u8* mem_data, size_t mem_size, size_t base_display_addr = 0x0000, std::vector<std::string>* labels = nullptr)
    {
        Sizes s;
        CalcSizes(s, mem_size, base_display_addr);
        ImGuiStyle& style = ImGui::GetStyle();
